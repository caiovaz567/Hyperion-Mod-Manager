...
- **End-to-end smoke test** (`npm run test:e2e`): launches the real app via Playwright, waits for the main window, walks every screen (Mods, Downloads, Settings, App Logs), screenshots each and fails on any renderer crash - the pre-release "does it even open" safety net. Navigation-only; it never touches mods, downloads or settings. The App Logs header button also gained a proper accessible name (`aria-label`).
