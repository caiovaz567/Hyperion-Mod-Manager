...
### Changed
- **The WolvenKit resource-hash database now loads during the splash and parses ~2x faster.** Conflict detection cross-references a bundled ~30 MB / 1.7-million-row hash database, which used to be parsed lazily on the first conflict pass — a ~1.5s blocking parse that landed right when the library and its conflict badges were coming up, and again on the first reinstall after launch. It is now preloaded during startup (while the splash is showing) so it's ready before you can interact, and the parse itself was rewritten to read the already-normalized CSV directly instead of re-running per-row normalization on all 1.7M rows (verified byte-for-byte identical output), cutting it from ~1.5s to ~0.8s. The parse also yields between chunks so it never freezes the app while loading. Net effect: conflict badges appear sooner on launch and reinstalls no longer stall waiting on a cold hash database.
