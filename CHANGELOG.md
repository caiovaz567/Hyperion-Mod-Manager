...
### Fixed
- **Disabling all mods no longer crashes the game (leftover files in the game folder).** Two leftover bugs are fixed. First, the bootstrap loaders Hyperion physically stages into `bin/x64` (CET's `version.dll`, ASI/RED4ext loaders) were only cleaned when Hyperion had *created* the file that run; once a loader survived one unclean exit it was treated as "already present" on the next launch and then never removed - so it piled up and a mod-free launch crashed on stale loaders. Cleanup now removes Hyperion's staged loaders outright in both cases - even if the mod was updated/uninstalled or the game touched the file at runtime - so no stray `.asi`/`.dll` lingers (a genuinely different file the user owns is still never touched). Second, emptied mod-framework folders (e.g. `bin/x64/plugins/cyber_engine_tweaks/mods`) that lingered after the game closed are now pruned, so closing the game really leaves the game folder clean; a vanilla (no-mods) launch also sweeps them first.
