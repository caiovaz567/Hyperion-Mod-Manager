...
- **Current coverage**: the entire renderer UI is now wired to `t()`/`tn()` — app shell (Header/Sidebar/App), Downloads, Library (toolbar, rows, mod-details, conflicts, dialogs), all Settings tabs, the shared install/conflict/version/duplicate/move-to-separator dialogs, the FOMOD installer, App Logs, and store-slice toasts. The remaining hardcoded English is limited to **main-process** error strings (`src/main/**`, surfaced via `IpcResult.error`); where the renderer supplies a fallback (`result.error ?? t(...)`), that fallback is translated. This now includes the Library action hooks (`useLibrary*Actions`/`useLibraryBulkToggle`/`useLibraryDragDrop`) whose toasts were previously hardcoded, plus the FOMOD parser display fallbacks, the `uiKit` validation-row empty label, and the Downloads delete-row badge. The remaining hardcoded English is limited to **main-process** error strings (`src/main/**`, surfaced via `IpcResult.error`); where the renderer supplies a fallback (`result.error ?? t(...)`), that fallback is translated. The **source-of-truth catalog is `en.json`** — every translated key lives there. `pt-BR.json` is now a **complete translation** at full key parity with `en.json` (712/712 keys). When adding UI strings, add the key to `en.json` first, then keep `pt-BR.json` at parity; any temporarily missing key still falls back to English at runtime, so a gap is safe but should be filled.
...
- The hash DB (`hashes.csv.gz`) is **preloaded during the splash** via `preloadHashDatabase()` in `loadHashDatabase()` (called from `app.whenReady` in `index.ts`, gated on a configured `libraryPath`), so the first conflict pass and any reinstall find it already parsed instead of paying ~1s on the user-interaction path — that lazy parse was what made conflict badges blank for a moment right after launch. `doLoadHashDatabase` parses the **already-normalized** CSV directly (single scan, no per-row `normalize*` helpers — those doubled parse time for a byte-for-byte identical map) and **yields to the event loop every `HASH_DB_PARSE_CHUNK` (100k) rows** so the splash-time preload never blocks IPC. The map is memoized (`hashDb`/`hashDbPromise`), so a concurrent deep pass awaits the same in-flight parse. Do not reintroduce the per-row normalization on the DB load path: the bundled CSV is guaranteed to be `<16-char lowercase FNV1a hex>,<forward-slash lowercase resource path>` with CRLF endings.
