...
- Update checking never does a full per-mod pass automatically. Update indicators are persisted between sessions and shown instantly from cache. On launch the app runs the cheap bulk check once in the background (silent, non-blocking): because the cache records when it last checked, this is normally a single `updated.json` request plus a detailed check only for the few mods that changed since the last open — never one request per mod. Beyond launch, refreshing is user-driven: the per-mod `Check for Update` right-click action, or the `Check Updates` toolbar button (same bulk pass, with a toast). Install/scan/reinstall/delete never trigger a check.
...
- Translation coverage is rolled out screen-by-screen. Untranslated strings fall back to English rather than showing missing-key placeholders, so a partially translated app always stays readable. Currently the setup wizard and Settings > General are fully localized; other screens follow in later work
