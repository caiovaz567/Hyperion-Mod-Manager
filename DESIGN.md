...
- A **collapsed** separator rolls its members' conflict state up onto the header row as **presence-only marks** in the same badge language - a green `+` (some member wins conflicts), a red `-` (some member is overwritten), a yellow `!` (some member is fully redundant) - deliberately with **no counts and no names**: on a large group any number is noise; the signal is just "there is overwriting going on inside". The marks appear only while collapsed and only for enabled members; the shared tooltip explains each mark in plain words and reminds that clicking expands the group, where the detailed per-mod badges live
